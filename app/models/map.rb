class Map
  ...
  def self.place_countries(grid)
    ...
    grid[5][2] = Country.where(name: :Colombia).first
    ...
    grid[6][1] = Country.where(name: :Peru).first
    grid[6][2] = Country.where(name: :Argentina).first
    grid[6][3] = Country.where(name: :Brazil).first
